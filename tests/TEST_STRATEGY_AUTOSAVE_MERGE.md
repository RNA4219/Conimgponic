...
7. `rm -rf coverage && pnpm -s test:coverage` / `pnpm test -- --test-reporter=junit --test-reporter-destination=file=reports/junit.xml` — スナップショット更新前と CI レポート収集時に実施。前者は `coverage/` を初期化してから実行することで JUnit 生成ステップに副作用を残さない。【F:tests/autosave/TEST_PLAN.md†L74-L78】【F:tests/cli/TEST_PLAN.md†L62-L80】
