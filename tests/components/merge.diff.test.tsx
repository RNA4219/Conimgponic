...
test('diff queue command returns conflict status when merge results in conflicts', async () => {
  // Create a storyboard with content that will result in conflicts
  const storyboardWithConflicts: Storyboard = {
    id: 'sb-conflict-test',
    title: 'Conflict Test',
    scenes: [
      {
        id: 'cut-conflict-1',
        manual: 'This is different from the AI content',
        ai: 'AI generated content that differs significantly',
        status: 'idle',
        assets: [],
        lock: null,
      },
      {
        id: 'cut-conflict-2',
        manual: 'Another different content',
        ai: 'Yet another different AI content',
        status: 'idle',
        assets: [],
        lock: null,
      }
  ...
  const harness = await renderStableDiffMergeDock({ autoSaveEnabled: true, storyboard: storyboardWithConflicts })

  try {
    // Execute the queue command and expect a conflict status
    const result = await harness.queue({
      type: 'queue-merge',
      precision: 'stable',
      origin: 'operation-pane.queue',
      hunkIds: ['cut-conflict-1', 'cut-conflict-2'],
      telemetryContext: {
        collectorSurface: 'diff-merge.operation-pane',
        analyzerSurface: 'diff-merge.queue',
        lastTab: 'diff',
      },
      metadata: { autoSaveRequested: false }, // Don't request autoSave for this test
    })

    // Verify that the result status is 'conflict' when there are conflicts
    assert.ok(result.status === 'conflict' || result.status === 'error', 
      `Expected 'conflict' or 'error' status when conflicts exist, got: ${result.status}`)
    
    // Also verify the telemetry includes retryable flag
    assert.ok(result.telemetry.retryable === true, 
      `Expected retryable flag to be true when conflicts exist, got: ${result.telemetry.retryable}`)
    
    // Verify hunk IDs are returned
    assert.ok(Array.isArray(result.hunkIds), 'Expected hunkIds to be an array')
    assert.ok(result.hunkIds.length > 0, 'Expected at least one hunk ID in result')
