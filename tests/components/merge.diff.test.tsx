...
test('beta precision without phase stats suppresses diff tab until guard lifts', () => {
  const plan = resolveMergeDockPhasePlan({
    precision: 'beta',
    threshold: 0.7,
  })
...
test('stable precision without phase stats defers diff until guard lifts', () => {
  const plan = resolveMergeDockPhasePlan({
    precision: 'stable',
    threshold: 0.82,
  })
...
test('stable precision retains diff merge preference across guard transitions', () => {
  ...
  assert.equal(sanitizePreference('manual-first', 'stable', guardedPlan.diff.enabled), 'diff-merge')
...
test('stable precision restores diff merge default when guard unlocks before manual override', () => {
  const guardedPlan = resolveMergeDockPhasePlan({
    precision: 'stable',
    threshold: 0.82,
  })
  ...
  assert.equal(restoredPreference, 'diff-merge')
