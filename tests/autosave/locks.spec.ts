...
baseScenario(
  'AS-LK-24: Fallback write failure publishes autosave.write.failed collector telemetry',
  {
    locks: {
      async request() {
        throw new DOMException('Lock already held', 'AbortError')
  ...
  async (t, ctx) => {
    ...
    const failure = new Error('Mock fallback write failure')
    failure.name = 'MockFallbackWriteFailure'
    ...
    assert.ok(failureEvents.length > 0, 'collector must capture autosave.write.failed event')
...
test('AS-LK-09d: releaseProjectLock failure invokes onReadonly once', async (t) => {
  ...
  const secondError = await assert.rejects(async () => releaseProjectLock(lease, { onReadonly }))
  assert.strictEqual(secondError, firstError)
