...
scenario(
  ...
  async (t) => {
    ...
    t.mock.timers.enable({ apis: ['setTimeout'], now: 0 })
    const uuids = [
      'lease-api-retry-success',
      'owner-api-retry-success',
      'lease-api-retry-fail',
      'owner-api-retry-fail',
    ]
    t.mock.method(crypto, 'randomUUID', () => {
      const value = uuids.shift()
      if (!value) throw new Error('uuid exhausted')
    ...
    const expectReleaseFailure = async (expectedCalls: number, delayMs?: number) => {
      const attempt = projectLockApi.release(lease)
      if (delayMs && delayMs > 0) {
        await Promise.resolve()
        t.mock.timers.tick(delayMs)
      }
    ...
    const thirdAttempt = projectLockApi.release(lease)
    await Promise.resolve()
    t.mock.timers.tick(1000)
    await thirdAttempt
    ...
    const secondLease = await projectLockApi.acquire({ preferredStrategy: 'web-lock', retry: false })
    const secondPlan = releaseRetryPlansForWebLocks[1]

    const expectReleaseRetry = async (expectedCalls: number, delayMs?: number) => {
      const attempt = projectLockApi.release(secondLease)
      if (delayMs && delayMs > 0) {
        await Promise.resolve()
        t.mock.timers.tick(delayMs)
      }
