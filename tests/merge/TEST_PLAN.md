...
| MG-U-04 | any | `AbortController` で Phase B 判定前に `abort()` / `abort('timeout')` を発火し、AutoSave キュー・イベントが停止するか。 | `merge3` が `MergeError` を throw し、`code/retryable/cause` が `aborted:false` / `timeout:true` となり Phase=queue フローへ未到達。 | 【F:src/lib/merge.ts†L932-L1002】 |
