...
| MG-U-05 | legacy | AutoSave 共有モック (`MockStoryboard`) を差分ソースに用い、初回 `merge3` 実行で `decision:'conflict'` を発生させた後に手動解決テキストへ置換して再実行する。 | 再実行で `stats.conflictDecisions===0` を満たしつつ、同じ手動解決シナリオを連続で実行しても `hunks`/`plan`/`trace.summary.threshold` とテレメトリ種別が不変。 | 【F:tests/merge/merge-engine.test.mjs†L74-L190】 |
