...
| MG-U-05 | legacy | AutoSave 共有モック (`MockStoryboard`) を差分ソースに用い、初回 `merge3` 実行で `decision:'conflict'` を発生させた後、`mergedText` を `ours` に反映しても再実行時に初回のハンク/計画/トレースとテレメトリ/イベントが変化しないかを検証。 | 再実行で `stats.conflictDecisions===0` を満たしつつ、初回結果の `hunks`/`plan`/`trace` とイベント配列が不変。 | 【F:tests/merge/merge-engine.test.mjs†L344-L463】 |
| MG-U-04 | any | `AbortController` で Phase B 判定前に `abort()` / `AbortSignal.timeout()` を発火し、AutoSave キュー・イベントが停止するか。 | `merge3` が `MergeError` を throw し、`code/retryable/cause` が `aborted:false` / `timeout:false` かつ queue フローへ未到達（`tests/autosave/TEST_PLAN.md` の非 retryable 区分と整合）。 | 【F:tests/merge/merge-engine.test.mjs†L1-L44】 |
...
| MG-I-02 | stable | `merge:autosave:lock` イベントを購読し、AutoSave ロック獲得/解放が Day8 Collector へ記録されるか統合試験。 | `events` が `merge:autosave:lock` を 2 回（acquired/released）送出し、Collector に `autosave.schedule.requested` が転送される。 | 【F:src/lib/merge.ts†L28-L78】【F:Day8/docs/day8/design/03_architecture.md†L3-L27】 |
