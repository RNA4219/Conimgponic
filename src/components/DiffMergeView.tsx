...
const resolveGlobalDiffMergeTabStorage=():DiffMergeTabStorage|undefined=>{const candidate=(globalThis as{localStorage?:unknown}).localStorage;if(!candidate)return undefined;const store=candidate as{getItem:(key:string)=>string|null;setItem:(key:string,value:string)=>void;removeItem?:(key:string)=>void};if(typeof store.getItem!=='function'||typeof store.setItem!=='function')return undefined;return{getItem:store.getItem.bind(store),setItem:store.setItem.bind(store),removeItem:typeof store.removeItem==='function'?store.removeItem.bind(store):undefined}}
