...
function buildTrace(
  ...
  const summary: MergeTraceSummary = {
    threshold: profile.threshold,
    autoAdoptionRate,
  };
  return {
...
export const DEFAULT_MERGE_ENGINE: MergeEngine = {
  merge3: (input: MergeInput, options?: MergeEngineOptions) => {
    ...
    try {
      ...
      const planResult: MergePlanResult = buildMergePlan(hunks, finalStats, profile, input.sceneId);
