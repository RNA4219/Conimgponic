 ...
 */
export const AUTOSAVE_POLICY: AutoSavePolicy = {
  debounceMs: 500,
  idleMs: 2000,
  maxGenerations: 20,
...
export const AUTOSAVE_STATE_TRANSITION_MAP: AutoSavePhaseTransitionMap = Object.freeze({
  ...
  dirty: ['debouncing:guard-enabled|phaseGuardEnabled=false で自動保存を再開', 'disabled:guard-disabled|ガード強制停止'],
...
export const AUTOSAVE_PHASE_DESCRIPTIONS: Readonly<Record<AutoSavePhase, AutoSavePhaseDescription>> = Object.freeze({
  ...
  dirty: { summary: 'Phase Guard により自動保存を保留中。', entry: ['pendingBytes を確定', 'guard.optionsDisabled を監視'], exit: ['phase guard を再評価', 'debounce/lock フローへ移行'] },
...
const createAutoSaveError = (
  ...
  return Object.assign(
    new Error(message),
    { name: 'AutoSaveError', code, retryable },
