...
- `HUB.codex.md` のタスク分割フローに従い、変更は 0.5 日未満で完了する最小単位に分解し、Public API を壊さない差分だけを扱う。
- `docs/TASKS.md` のテンプレートを使い、Plan/Patch/Tests/Commands/Notes を更新して差分理由と影響範囲を記録する。既存チェックリストに沿ってレビュー前に検証ログを残す。
- 実装は必ずテスト起点で進め、失敗する RED テストを先に追加し、GREEN 化後も Tests セクションへ実行結果を転記する。

## プロセスと自己検証

- 競合解消時は双方の意図を最小限で統合し、判断を `ノート→` に 1 行で記す。
- 差分提示前に lint/type/test をメンタルで実行し、グリーン想定の変更のみ提出する。
- Day8/docs/day8/guides/07_contributing.md と整合を取り、**1タスク=1ブランチ=1PR**の分割と、テストを先に書く TDD フロー（RED→GREEN→リファクタ）のログを `Tests` セクションへ残す。
