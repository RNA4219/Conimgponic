...
9. **タスク化**: タスクは独立性が保てる粒度まで分割し、責務の重複(コンフリクト)を避ける。
   ...
   既存ハンク選択・編集中に新規ハンクが同期されても状態が保持されることを Task Seed 要件へ明示し、検証ログとチェックリストを連動させる。
   StoryboardList 数値バリデーション改善タスクでは、Seed に `pnpm test --filter components -- --test-name-pattern StoryboardList.numeric-input` の実行手順と、`take` / `seed` フィールドで非数値入力時に旧値が保持される確認ログを必ず追記する。
