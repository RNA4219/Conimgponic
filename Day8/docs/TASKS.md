...
- DiffMerge タブ永続化改善タスクでは、`tests/components/DiffMergeView.localStorage-remove.spec.tsx` を用いた `pnpm test --filter components -- --test-name-pattern DiffMergeView.localStorage-remove` の実行ログと、`localStorage.removeItem` 例外時に初期タブへフォールバックした記録を Notes/Tests に追加する。
