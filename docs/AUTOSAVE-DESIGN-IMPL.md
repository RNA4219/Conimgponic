...
- `flag_resolution.threshold` には `resolveFlags()` 正常化済みの数値のみを流す。`merge.precision` が env/workspace/localStorage で誤設定された場合も `FlagValidationError` を Collector へ積み増し、`DEFAULT_FLAGS.merge.profile.threshold` へフォールバックして Phase ガード要件と矛盾しないテレメトリを保証する。
