...
| `merge.trace` | `payload.phase`, `payload.collisions`, `payload.processing_ms`, `payload.guardrail.metric`, `payload.guardrail.observed`, `payload.guardrail.tolerance_pct`, `payload.guardrail.rollbackTo`, `payload.digest` | `processing_ms` を P95 化し、`guardrail.metric` (`merge_auto_success_rate` / `merge_processing_p95`) を Analyzer の Phase B 判定へ転送 | `observed` 超過が 2 バッチ続いたら `payload.guardrail.rollbackTo` へフェーズ差し戻し | `digest` は Analyzer のハッシュ衝突を防ぐ監査キー |
...
Analyzer への 15 分窓集計では Collector が `ui_saved_rate`・`merge_auto_success_rate`・`merge_processing_p95`・`export_latency_p95`・`export_success_rate` を `payload.guardrail.metric` / `payload.summary.*` から正規化し、Phase A/B/Export のガード閾値と突合する。これらのメトリクスが欠損した場合は Analyzer 側で Rollback 判定を継続できないため、Collector は JSON Schema 検証で異常を検知して再投入を待つ。
