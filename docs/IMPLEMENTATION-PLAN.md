...
| Merge 精度制御 | `VITE_MERGE_PRECISION` → `conimg.merge.threshold` | `localStorage.merge.precision` | `merge.precision='legacy'` | `flag_resolution` に `precision` と `threshold` を同梱し、`DiffMergeView` 露出率を追跡。 | Phase C で `beta/stable` を解放し、しきい値は `DEFAULT_THRESHOLD=0.75` を下回らないよう監視する。【F:docs/MERGE-DESIGN-IMPL.md†L140-L206】【F:docs/AUTOSAVE-DESIGN-IMPL.md†L19-L85】 |
...
| `stable` | `Diff` 初期タブ | `merge.request`・既存履歴スナップショット | `merge.result` + `traceId` | `merge.trace`（hash/evidence） | 精緻モード。タブ初期表示で Diff Merge を前面化。【F:docs/IMPLEMENTATION-PLAN.md†L150-L189】 |
| `stable` | `OperationPane` / `EditModal` / `BulkPane` | 選択ハンクとユーザ操作 | 編集結果 `merge.command`、`rollbackCommand`、Evidence 更新 | `merge.command` / `merge.rollback` | `EditModal` の保存は AutoSave ポリシーに倣い 50MB/20世代制限に従う。【F:docs/AUTOSAVE-DESIGN-IMPL.md†L33-L104】 |
...
- **解放要件の整理**: Web Lock の解放は `releaseProjectLock` が Web Lock ハンドルの `release()`／`released`／`navigator.locks.request` 完了を直列で待機し、`releaseDeferred`→`requestSettled`→`lock:released` の順で収束させる。追加モニタリング（`releaseMonitor` など）に依存せず、失敗時は `lock:error(retryable=true)` → `lock:readonly-entered(reason='release-failed')` を発火してフォールバック削除とロールバック判定を同期する（`docs/AUTOSAVE-DESIGN-IMPL.md` §3.1）。
- **Release 再試行の扱い**: Web Lock 解放失敗時はリース ID ごとに試行回数と待機時間をキャッシュし、`0.5s → 1.0s → 2.0s` の指数バックオフで最大 3 回まで `releaseProjectLock` を再実行する。待機中の呼び出しは Delay 解消まで待機し、成功時に状態を破棄する。【F:docs/AUTOSAVE-DESIGN-IMPL.md†L812-L876】
- **Readonly 降格の継続**: 初回失敗で `lock:readonly-entered(reason='release-failed')` / `onReadonly` を通知しつつ、成功するまで同一エラーを共有する。3 回失敗した場合は同一 `ProjectLockError` を即時再送出して Runbook の降格判定に接続する。【F:docs/tasks/task-autosave-project-locks.md†L13-L30】
...
stateDiagram-v2
    ...
    Releasing --> Readonly: release-failed / retryable=true (lastError)
...
- [x] `Collector` へのテレメトリ送信がフラグ ON/OFF 双方で同一スキーマ（JSONL）を維持する統合テスト。 (2024-06-19, [検証ログ: tests/platform/vscode/autosave/autosave.responsibility.test.ts](../tests/platform/vscode/autosave/autosave.responsibility.test.ts))
- [x] `Collector` へのテレメトリ送信がフラグ ON/OFF 双方で同一スキーマ（JSONL）を維持する統合テスト。 (2024-05-06, [検証ログ: tests/platform/vscode/autosave/autosave.responsibility.test.ts](../tests/platform/vscode/autosave/autosave.responsibility.test.ts), [tests/platform/vscode/autosave/autosave.collector-export.test.ts](../tests/platform/vscode/autosave/autosave.collector-export.test.ts))
